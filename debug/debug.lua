...
function export.dump(value, prefix, tsort)
	...
	elseif t == "table" then
		...
		for key, val in require("table.lua").sortedPairs(value, tsort) do
...
local function containsTablesWithSize(t, size)
	for key, value in pairs(t) do
		if type(value) == "table" and require("table.lua").size(value) ~= size then
			return false
		end
	end
	return true
...
function export.modified_dump(value, prefix, tsort)
	...
	elseif t == "table" then
		...
		local consecutive = require("table.lua").isArray(value)
