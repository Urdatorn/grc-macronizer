...
local data = require('grc-conj.data.lua')
local m_accent = require('grc-accent.lua')
local m_links = require('links.lua')
...
local function get_label_display(dial)
	return require("labels.lua").get_label_info { label = dial, lang = lang, nocat = true }.label
end
...
function export.show(frame)
	...
	elseif not args[1]:find('irreg') then
		...
		if malformed then
			...
			error('Malformed input in parameter' .. (plural and 's ' or ' ') 
				.. require('table.lua').serialCommaJoin(malformed) .. ' (contains extra accents)')
		end
	end
	...
	if mw.ustring.find(stem1, '˘') then
		require('debug.lua').track('grc-conj/manual-breve')
	end
	if args['dial1'] then require('debug.lua').track('grc-conj/dial1') end
	if args['titleapp'] then require('debug.lua').track('grc-conj/titleapp') end
	if args['prefix'] then
		require('debug.lua').track('grc-conj/prefix')
	...
	if not conjugations[conjtype] then
		...
		if tense_code then
			if tense_names[tense_code] then
				local valid_conjugations_for_tense = require('array.lua')
					.keys(conjugations)
					:filter(
				...
				error('No such tense: ' .. tense_code .. '; choose between '
					.. require('array.lua').keys(tense_names)
						:map(
							function (tense_code)
...
conjugations['aor-amiw'] = function()
	require('debug.lua').track('grc-conj/aor-amiw')
...
conjugations['aor-ami'] = function()
	require('debug.lua').track('grc-conj/aor-ami')
	ctable = data.aor_ami
	pctable = data.aor_pass
	pstem = stem3
	pstem2 = stem4
...
conjugations['aor-hmi'] = function()
	require('debug.lua').track('grc-conj/aor-hmi')
	ctable = data.aor_hmi
	pctable = data.aor_pass
	pstem = stem3
	pstem2 = stem4
...
function make_table()
	...
	return require('TemplateStyles.lua')('Module:grc-conj/style.css')
.. [=[<div class="NavFrame">
...
local get_contraction_map = require "fun.lua".memoize(function (conjugation_code)
...
function link_form(form_code, istitle)
	...
	if args[form_code] then
		form = args[form_code]
		require('debug.lua').track('grc-conj/form-override')
