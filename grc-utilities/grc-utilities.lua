...
local sc = require("scripts.lua").getByCode("Polyt")

local m_data = mw.loadData("grc-utilities.data.lua")
...
local sparseConcat = require("table.lua").sparseConcat
	...
	forEach(diacritics,
		function (diacritic)
			...
			if not output[index] then
				...
				require("debug.lua").track("grc-utils/too many diacritics")
				--[[
				local m_templates = require("grc-utilities.templates.lua")
			...
			elseif prev_info == rho_t then
				if curr_info ~= breathing_t then
					mw.log(string.format("The character %s in %s should not have the accent %s on it.",
						prev, text, require("grc-utilities.templates.lua").addDottedCircle(character)))
				end
			else
